...
procedure OverloadDetection is
   ...
   task type Helper(Prio : Integer; Phase : Integer; Period : Integer; Relative_Deadline: Integer) is
      pragma Priority(Prio);
   end;
   
   task type Watchdog(Phase : Integer; Period : Integer; Relative_Deadline : Integer) is
      pragma Priority(30);
      entry SendSignal;
   end;

   Watchdog_1 : Watchdog(Warm_Up_Time, 12000, 12000); -- (Phase, Period, Relative_Deadline)
   ...
   begin
      -- Initial Release - Phase
      Release := Clock + Milliseconds(Phase) + Milliseconds(Relative_Deadline);
      delay until Release;
      loop
         Release := Release + Milliseconds(Period);
         Signal := 0;
         Put_Line("");
         Put_Line("Watchdog is executing...");
   ...
   Task_1 : T(1, 20, Warm_Up_Time, 3000, 1000, 3000);
   Task_2 : T(2, 15, Warm_Up_Time, 4000, 1000, 4000);
   Task_3 : T(3, 10, Warm_Up_Time, 6000, 1000, 6000);
   Task_4 : T(4, 5, Warm_Up_Time, 12000, 2000, 12000);
   
   Helper_Task_1 : Helper(1, Warm_Up_Time, 12000, 12000); -- (Prio, Phase, Period, Relative_Deadline)
