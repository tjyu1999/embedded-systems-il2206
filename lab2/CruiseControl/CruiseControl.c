...
void switch_timer_callback(){
    OSSemPost(switch_semaphore);
}
...
void detection_timer_callback(){
    OSSemPost(detection_semaphore);
}

void extraload_timer_callback(){
...
void ControlTask(void* pdata){
    ...
    while(1){
        ...
        int gled = IORD_ALTERA_AVALON_PIO_DATA(DE2_PIO_GREENLED9_BASE);
        throttle = 40;
...
void ButtonIOTask(void* pdata){
    ...
    while(1){
        ...
        if(buttons & CRUISE_CONTROL_FLAG){
            if(cruise_control == on){
                cruise_control = off;
                IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_GREENLED9_BASE, gled ^ 0x00000004);
            }
            else{
                cruise_control = on;
                IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_GREENLED9_BASE, gled | 0x00000004);
        ...
        // brake pedal
        else if(buttons & BRAKE_PEDAL_FLAG){
            if(brake_pedal == on){
                brake_pedal = off;
                IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_GREENLED9_BASE, gled ^ LED_GREEN_4);
        ...
        // gas pedal        
        else if(buttons & GAS_PEDAL_FLAG){
            if(gas_pedal == on){
                gas_pedal = off;
                IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_GREENLED9_BASE, gled ^ LED_GREEN_6);
            }
            else{
                gas_pedal = on;
                IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_GREENLED9_BASE, gled | LED_GREEN_6);
...
void SwitchIOTask(void* pdata){
    ...
    void* msg;
    INT16S* current_velocity;
    INT16S* tmp_current_velocity;  

    while(1){
        OSSemPend(switch_semaphore, 0, &err);
        switches = switches_pressed();
        
        msg = OSMboxAccept(Mbox_Engine);
        if(msg == NULL) engine = off;
        else engine = (enum active) msg;
